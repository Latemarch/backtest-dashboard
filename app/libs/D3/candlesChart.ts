...
type Props = {
  ...
  yMACD: d3.ScaleLinear<number, number, never>;
  yMACD2: d3.ScaleLinear<number, number, never>;
  yRSI: d3.ScaleLinear<number, number, never>;
  width: number;
  height: number;
  ...
  yMACDAxisGroup: d3.Selection<SVGGElement, unknown, null, undefined>;
  yMACD2AxisGroup: d3.Selection<SVGGElement, unknown, null, undefined>;
  yRSIAxisGroup: d3.Selection<SVGGElement, unknown, null, undefined>;
};
...
export function updateAxis({
  ...
  const yMACDAxis = d3
    .axisRight(yMACD)
    .ticks(3)
    .tickFormat((d: any) => (Math.abs(d) === 100 ? "" : d))
    .tickSizeInner(-width);
  const yMACD2Axis = d3
    .axisRight(yMACD2)
    .ticks(3)
  ...
  const yRSIAxis = d3
    .axisRight(yRSI)
    .ticks(4)
    .tickFormat((d: any) => (d % 100 === 0 ? "" : d))
    // .tickFormat((d: any) => (d === 0 ? '' : d.toFixed(2)))
    .tickSizeInner(-width);

  xAxisGroup.call(xAxis);
  yAxisGroup.call(yAxis);
  yVolumeAxisGroup.call(yVolumeAxis);
  yMACDAxisGroup.call(yMACDAxis);
  yMACD2AxisGroup.call(yMACD2Axis);
  yRSIAxisGroup.call(yRSIAxis);

  xAxisGroup.selectAll("path").remove();
  yAxisGroup.selectAll("path").remove();
  yVolumeAxisGroup.selectAll("path").remove();
  yMACDAxisGroup.selectAll("path").remove();
  yMACD2AxisGroup.selectAll("path").remove();
  yRSIAxisGroup.selectAll("path").remove();
  svg
    .selectAll(".tick line")
...
export function createBaseLine(
  ...
  const splitLineX2 = svg
    .append("line")
    .attr("class", "base-line-x2")
    .attr("x1", 0)
    .attr("y1", height * macdChartHeightRatio)
    .attr("x2", width * 5)
    .attr("y2", height * macdChartHeightRatio)
    .style("stroke", colors.baseLine)
    .style("stroke-width", 1);

  const splitLineX3 = svg
    .append("line")
    .attr("class", "base-line-x3")
    .attr("x1", 0)
    .attr("y1", height * macd2ChartHeightRatio)
    .attr("x2", width * 5)
    .attr("y2", height * macd2ChartHeightRatio)
    .style("stroke", colors.baseLine)
    .style("stroke-width", 1);

  const splitLineX4 = svg
    .append("line")
    .attr("class", "base-line-x4")
    .attr("x1", 0)
    .attr("y1", height * rsiChartHeightRatio)
    .attr("x2", width * 5)
    .attr("y2", height * rsiChartHeightRatio)
    .style("stroke", colors.baseLine)
    .style("stroke-width", 1);
